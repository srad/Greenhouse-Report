...
The changes made in this section will mainly focus on the real properties of the leave's topside(the side that faces the sun). This is based on the fact, that the leafarea is calculated by taking a snapshot from the top of the plant. The properties of bottom-side are irrelevant and can be neglected in the simulation.
...
Since all leaves have been detected, a new material needs to be established. As described in the previous section the color, metallic, specularity and roughness of the material will be edited, so the leaf gets the structure as described. The parameters of the diffuse_color describe the red, green, blue and alpha-Values. The parameters of all other shown properties can be changed to values between zero and one. So the Material can delicately be editet.
\lstset{language=Python, frame=single}
\begin{lstlisting}
